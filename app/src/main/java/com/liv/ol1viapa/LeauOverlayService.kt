...
class LeauOverlayService : Service() {
    ...
    private fun showBubble() {
        ...
        val b = ImageView(this).apply {
            ...
            background = bg(Color.rgb(19, 27, 28), 100f)
        ...
        wm.addView(r, layoutParams(76, 76))
    }
    ...
    private fun showPanel() {
        ...
        val card = LinearLayout(this).apply {
            ...
            setOnTouchListener { _, e ->
                if (e.action == MotionEvent.ACTION_DOWN || e.action == MotionEvent.ACTION_MOVE) {
                    // Drag from the panel keeps the popup movable.
                    if (e.action == MotionEvent.ACTION_MOVE) move(e.rawX, e.rawY)
                }
                false
            }
        ...
        val icon = ImageView(this).apply { setImageResource(R.drawable.leau_eyes); setPadding(2,2,2,2) }
        header.addView(icon, LinearLayout.LayoutParams(40, 40))
        ...
        val input = EditText(this).apply {
            hint = "Ask LEAU anything..."; textSize = 15f; setSingleLine(true)
            setTextColor(Color.WHITE); setHintTextColor(Color.rgb(160,170,170)); setPadding(16,0,16,0)
            background = bg(Color.rgb(45,54,55), 40f)
        }
        ...
        card.addView(row)
        r.addView(card, FrameLayout.LayoutParams(310, -2).apply { leftMargin = 8; topMargin = 8 })
        input.requestFocus()
    ...
    private fun move(rawX: Float, rawY: Float) {
        ...
        root?.let { p -> (p.layoutParams as WindowManager.LayoutParams).also { it.x=x; it.y=y; wm.updateViewLayout(p,it) } }
    }
