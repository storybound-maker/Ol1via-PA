...
class LeauCallActivity : Activity() {
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (grantResults.isNotEmpty() && grantResults.all { it == PackageManager.PERMISSION_GRANTED }) placeCall()
        else placeCall(useDialerFallback = true)
    }
    ...
    private fun placeCall(useDialerFallback: Boolean = false) {
        ...
        val action = if (!useDialerFallback && checkSelfPermission(Manifest.permission.CALL_PHONE) == PackageManager.PERMISSION_GRANTED) Intent.ACTION_CALL else Intent.ACTION_DIAL
    ...
    private fun resolveNumber(value: String): String? {
        if (looksLikePhoneNumber(value)) return value.replace(Regex("[^0-9+*#]"), "")
        ...
        var bestScore = 0.0
        contentResolver.query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI, projection, null, null, ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME + " COLLATE NOCASE ASC")?.use { cursor ->
    ...
    private fun looksLikePhoneNumber(value: String): Boolean = value.count { it.isDigit() } >= 7 && value.count { it.isLetter() } <= 1

    private fun normalize(value: String): String = value.lowercase(Locale.US).replace("’", "'").replace(Regex("[^a-z0-9]+"), " ").replace(Regex("\\s+"), " ").trim()
